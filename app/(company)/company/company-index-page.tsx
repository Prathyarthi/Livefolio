  ...
  if (isLoading) {
    return (
      <div className="flex min-h-0 flex-1 flex-col bg-surface-base">
        <LandingNav variant="recruiter" sections={false} />
        <div className="mx-auto w-full max-w-4xl space-y-8 px-6 py-10 md:py-14">
          <header className="space-y-2">
            <p className="eyebrow uppercase">Hiring</p>
            <h1 className="text-h2 text-text-primary">Companies</h1>
          </header>
          <p className="text-body-sm text-text-muted">Loading companies…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-surface-base">
      ...
      <div className="mx-auto w-full max-w-4xl space-y-8 px-6 py-10 md:py-14">
        ...
        {!showCreate && orgs && orgs.length > 0 ? (
          ...
          <div className="space-y-6 rounded-[var(--radius-lg)] border border-border-default bg-surface-raised p-6 shadow-[var(--shadow-card)]">
            <h2 className="text-h3 text-text-primary">
              {orgs && orgs.length > 0 ? "New company" : "No organization yet"}
            </h2>
            {!(orgs && orgs.length > 0) ? (
              <p className="text-body-sm text-text-secondary">
        ...
        <p className="text-body-sm text-text-muted">
          ...
          <Link href="/dashboard" className="font-medium text-brand-secondary underline-offset-4 hover:underline">
            Go to dashboard
          </Link>
        </p>
      </div>
